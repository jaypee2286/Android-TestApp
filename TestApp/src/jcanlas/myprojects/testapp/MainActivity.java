    ...
    private class readTeamsJSONFeedTask extends AsyncTask<String, Void, String> {
    	...
    	protected void onPostExecute(String result) {
    		...
    		try {    			
    			...
    			JSONArray sportsArray = new JSONArray(sports.getString("sports")); 			//"sports" array
				JSONObject sportsObject = sportsArray.getJSONObject(0); 					//"sports" objects
					JSONArray leaguesArray = new JSONArray(sportsObject.getString("leagues"));	//"leagues" array
					JSONObject leaguesObject = leaguesArray.getJSONObject(0);					//"leagues" objects
						JSONArray teamsArray = new JSONArray(leaguesObject.getString("teams"));		//"teams" array
				...
				for(int i = 0; i < teamsArray.length(); i++){
					...
					db.addTeam(new Team(teamsObject.getString("location"), teamsObject.getString("name")));	
					
					// for display in android app textview
					teamListArray.add(teamsObject.getString("location") + " " + teamsObject.getString("name"));
		        ...
		        for (int j = 0; j < teams.size(); j++) {
		            ...
		            String log = "Id: "+ team.getID() +" ,Location: " + team.getLocation() + " ,Name: " + team.getName();
		            Log.d("Name: ", log);
		        }
